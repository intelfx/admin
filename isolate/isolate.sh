...
#
# nvidia GPU support (hack)
#

nvidia_setup() {
	local LIBSH_LOG_PREFIX="qemu::nvidia_setup($GUEST_NAME)"
	local dev nvidia_unbind
...
nvidia_teardown() {
	local LIBSH_LOG_PREFIX="qemu::nvidia_teardown($GUEST_NAME)"
	local dev nvidia_rebind

	STATE_FILE="$STATE_DIR/nvidia_guests"
	if [[ -e "$STATE_FILE" ]]
	then eval "$(< "$STATE_FILE" )"
	else warn "state file does not exist: $STATE_FILE"; declare -A nvidia_guests
	fi
	nvidia_rebind=0
